...
public class ERA5 {
  ...
  static final String outputDir = "E:/Jobs/Tori-Brazil/";
  ...
  public static void main(String[] args) throws Exception {
    /*
  	DataInputStream dis = new DataInputStream(new BufferedInputStream(new FileInputStream(
        "C:\\Files\\Dev\\vs2022\\HDFConvert\\HDFConvert\\out.bin")));
    ...
    for (int j=0; j<21600; j++) {
      for (int i=0; i<43200; i++) {
        ...
        short sh = Short.reverseBytes(dis.readShort());
        if ((i == 10574) && (j == 6427)) System.out.println(z+": "+sh);        
        if ((sh >= 0) && (sh <= 10000))
          bi.setRGB(i/10,  j/10,  cols[(sh*255)/10000]);
      }
    }
    ImageIO.write(bi,  "PNG",  new File("E:/Stuff/test.png"));
    dis.close();
    System.exit(0);
    */
   // GlobalRasterTools G = new GlobalRasterTools();
   // int[][] pop = G.loadIntGrid("E:/Data/Census/Landscan2014/Landscan2014.bil",43200,21600,43200,21600,0,0);
   // long sum=0;
   // for (int j=0; j<21600; j++)
   //   for (int i=0; i<43200; i++)
   //     sum+=pop[j][i];
   ...
   // System.exit(0);
    ...
    args = new String[] {"U:/Wes/Climate/", "Brazil3", "shp", "2013", "2020"};
    ...
    if (args[2].equals("pull")) {
      ...
      GRT.loadPolygons(0, shp, dbf, null, "TORI_BRAZIL2");
